...
Key improvements made (see code comments):
...
NOTE: This still uses polling on the main thread. For production / many instruments
it's recommended to use Upstox WebSocket V3 for real-time updates instead of polling.
"""
...
def option_chain_json_to_df(json_obj: Dict[str, Any]) -> pd.DataFrame:
    # Support either raw response {"data": [...]} or wrapper {"snapshot": {"data": [...]}}
